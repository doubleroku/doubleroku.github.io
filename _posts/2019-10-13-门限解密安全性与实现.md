...
5. 解密份额的有效性可以被检测，即成员中的欺骗者可以被识别。
   ...
   论文中提出一个门限密码方案，是基于在椭圆曲线上使用双线性映射构造的GDH群实现的。
...
- 因此Type a中的方案，就是通过修改后的Tate对，构造了一个椭圆曲线上的双线性映射，实现了GDH的性质。
  ...
  **$\mathrm{K}(k, n, t)$**：
  ...
  输入：安全参数$k$，参与协议的解密服务器数$n$，门限参数$t$，算法从$\mathbb{Z}_{q}^{*}$中均匀随机地选取$a_{0}, a_{1}, \dots, a_{t-1}$，定义一个多项式$\operatorname{Poly}(X)=\sum_{j=0}^{t-1} a_{j} X^{j}$。对$0 \leq i \leq n$，计算$x_{i}=P o l y(i) \in \mathbb{Z}_{q}^{*}$，$Y_{i}=x_{i} P$，$x \stackrel{\text { def }}{=} a_{0}=\operatorname{Poly}(0)$，$Y \stackrel{\text { def }}{=} Y_{0}=x P$。
