...
门限密码系统是在秘密分享方案的基础上构建出来的一种加密原语。在一个$(t,n)$的门限密码方案中，私钥被划分为$n$个份额，每个份额都被分配给不同的参与解密的服务器。使用对应公钥加密得到的密文$c$，至少需要$t \le n$个服务器参与解密，各自提交正确的份额给$Combiner$才能够恢复出正确的明文，从而能够在防止单点故障、实现权力分散等方面得到应用。
