...
$\textbf{Combine}(PK, \mathbf{VK}, C,{\mu_{i}})$：输入公钥$PK$，验证密钥$VK$，密文$C$，对于集合$$S \subset\{1, \ldots, n\}$$，且有$|S|≥t$，$S$中每一个参与者输入各自的解密份额${\mu_{i}}$。如果该集合中的参与者提交了无效的解密份额，使得${Share-Verify}(PK,VK_i,C,\mu_{i})=0$，则算法输出$\perp$，否则输出明文$M$。
